...
<div class="container py-5">
    ...
    <form action="{{route('restaurants.update',$restaurant)}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" pattern="[A-Za-z ]+" title="Inserisci un indirizzo" required class="form-control @error('address') is-invalid @enderror" value="{{old('address', $restaurant['address'])}}"  id="exampleFormControlInput1" name="address">
                @error('address')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
        </div>
